...
export default function CommentSection({ postId }) {
  ...
  const handleLike = async (commentId) => {
    try {
      ...
      const res = await axios.put(import.meta.env.VITE_BACKEND_URL+`/api/comment/likeComment/${commentId}`,{},{headers:{"Authorization":`Bearer ${currentUser.token}`,'Content-Type': 'application/json'}},{withCredentials:true})
      const data = res.data;
      if(data?.unAuthorized && data.unAuthorized){
        toast.error(data.message)
      }
  ...
  const handleDelete = async (commentId) => {
    ...
    try {
      ...
      const res = await axios.delete(import.meta.env.VITE_BACKEND_URL+`/api/comment/deleteComment/${commentId}`,{headers:{"Authorization":`Bearer ${currentUser.token}`,'Content-Type': 'application/json'}},{withCredentials:true})
      const data = res.data;
      if(data?.unAuthorized && data.unAuthorized){
        toast.error(data.message)
      }
